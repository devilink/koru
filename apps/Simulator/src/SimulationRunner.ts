...
async function runSimulation() {
  ...
  rules.registerRule('HARDWARE:BATTERY_UPDATED', (payload: any) => {
    if (payload.level < 15) return { Energy: -30, Stress: +20, Sleepiness: +40 };
    return {};
  });

  rules.registerRule('PLANT:HEALTH_UPDATED', (payload: any) => {
    if (payload.status === 'unhealthy') return { PlantConcern: +50, Stress: +30 };
    return {};
  });
  ...
  anims.registerAnimation('SLEEP', 1, (s: EmotionState) => s.Sleepiness > 80);
  anims.registerAnimation('CRY', 2, (s: EmotionState) => s.Stress > 80);
  anims.registerAnimation('UNCOMFORTABLE', 3, (s: EmotionState) => s.PlantConcern > 70 || s.Stress > 60);
  anims.registerAnimation('LOVE', 4, (s: EmotionState) => s.Friendship > 80 && s.Stress < 30);
